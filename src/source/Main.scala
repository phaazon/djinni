...
object Main {
  ...
  def main(args: Array[String]) {
    ...
    var reactNativeObjcOutFolder: Option[File] = None
    var reactNativeTypePrefix: String = ""
    var reactNativeObjcImplSuffix: String = ""
    var reactNativeJavaOutFolder: Option[File] = None
    var reactNativeJavaPackage: Option[String] = None
    var exportHeaderName: String = ""
    ...
    val argParser = new scopt.OptionParser[Unit]("djinni") {
      ...
      //React Native
      opt[File]("react-native-objc-out").valueName("<out-folder>").foreach(x => reactNativeObjcOutFolder = Some(x))
      ...
      opt[String]("react-include-objc").valueName("<prefix>").foreach(reactIncludeObjc = _)
        .text("The relative path from react-native-objc-out to objc-out")
      opt[String]("react-include-objc-impl").valueName("<prefix>").foreach(reactIncludeObjcImpl = _)
        .text("The relative path from react-native-objc-out to objc implementation directory")
      ...
      opt[File]("react-native-java-out").valueName("<out-folder>").foreach(x => reactNativeJavaOutFolder = Some(x))
        .text("The output folder for React Native (Java) files (Generator disabled if unspecified)")
      opt[String]("react-native-java-package").valueName("...").foreach(x => reactNativeJavaPackage = Some(x))
        .text("The package name to use for generated React Native Java bridge classes.")
      /*
      ...
      */
      opt[String]("export-header-name").valueName("<pre>").foreach(exportHeaderName = _)
        .text("The name of export header generated by CMake command")
    ...
    preprocessor.resolveTemplates(meta.defaults, idl) match {
      case Success((meta, idl)) =>
        ...
        val outSpec = Spec(
          ...
          reactNativeObjcOutFolder,
          reactNativeTypePrefix,
          reactNativeObjcImplSuffix,
          reactNativeJavaOutFolder,
          reactNativeJavaPackage,
          exportHeaderName,
          traceMethodsCalls)
